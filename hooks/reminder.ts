...
export const useReminder = () => {
  ...
  }, [loading, currentPage, currentFilter]);
  ...
  const fetchCounts = async () => {
    try {
      ...
      const filters = ["all", "sent", "scheduled", "pending"];
      const countPromises = filters.map(async (filter) => {
        const url = filter === "all" 
          ? `${API_URL}/reminders?page=1&limit=1`
          : `${API_URL}/reminders?filter=${filter}&page=1&limit=1`;
        ...
        if (response.ok) {
          const data = await response.json();
          return { filter, count: data.total || 0 };
        }
        return { filter, count: 0 };
      });

      const results = await Promise.all(countPromises);
      const newCounts = results.reduce((acc, { filter, count }) => {
        acc[filter as keyof typeof counts] = count;
        return acc;
      }, { all: 0, sent: 0, scheduled: 0, pending: 0 });
  ...
  const fetchReminders = async () => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/reminders?page=${currentPage}&limit=${limit}${filterParam}`,
        {
          headers: {
            "Authorization": `Bearer ${token}`,
            "Content-Type": "application/json",
