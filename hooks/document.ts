...
export const useDocuments = () => {
  ...
  const handleFileSelect = (file: File | null) => {
    if (file) {
      ...
      setUploadForm(prev => ({ ...prev, doc_name: file.name }));
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (uploadForm.app_id) formData.append('app_id', uploadForm.app_id);
  ...
  const handleDownload = async (doc: Document) => {
    ...
    try {
      const res = await fetch(`http://localhost:4000/document/${doc.doc_id}/download`, {
        headers: { Authorization: `Bearer ${token}` }
      });
